...
#pragma HLS INTERFACE m_axi depth = 16384 port = data_in0 offset = slave bundle = in1
#pragma HLS INTERFACE m_axi depth = 16384 port = data_in1 offset = slave bundle = in2
#pragma HLS INTERFACE m_axi depth = 16384 port = data_in2 offset = slave bundle = in3
#pragma HLS INTERFACE m_axi depth = 81920 port = data_out0 offset = slave bundle = in3
#pragma HLS INTERFACE m_axi depth = 49152 port = rp_in0 offset = slave bundle = in2


	//--------Modulus DATA init--------------------------------------------------
	UDTYPE modulus[MODCOUNT1][3] = {{66404353, 11391767, 4},
									{66420737, 11124097, 4},
									{66551809, 8987482, 4},
									{66813953, 4739403, 4},
									{66961409, 2364475, 4},
	...
	UDTYPE half_layer1[MODCOUNT1] = {33497088, 33497088, 33497088, 33497088, 33497088, 33497088};
...
#pragma HLS ARRAY_PARTITION variable = input_data complete dim = 3
	...
	multplain_inplace_cp_mc3(data_in0 + idx_data, data_in1 + idx_data, data_in2 + idx_data,
							 input_data[5], input_data[4], input_data[3],
							 modulus[5], modulus[4], modulus[3]);

	intt_mc3_new(input_data[5], input_data[4], input_data[3],
				 idt[5], idt[4], idt[3],
				 sidt[5], sidt[4], sidt[3],
				 modulus[5][0], modulus[4][0], modulus[3][0]);

	rescale_para_mc2<1>(input_data[5], input_data[4], input_data[3],
						half_layer1[5], half_layer1[4], half_layer1[3],
						inv[4], inv[3],
						modulus[5], modulus[4], modulus[3]);

	ntt_mc2_new(input_data[4], input_data[3],
				rp[4], rp[3],
				srp[4], srp[3],
	...
	multplain_inplace_cp_mc3(data_in0 + idx_data, data_in1 + idx_data, data_in2 + idx_data,
							 input_data[2], input_data[1], input_data[0],
							 modulus[2], modulus[1], modulus[0]);

	intt_mc3_new(input_data[2], input_data[1], input_data[0],
				 idt[2], idt[1], idt[0],
				 sidt[2], sidt[1], sidt[0],
				 modulus[2][0], modulus[1][0], modulus[0][0]);

	rescale_para_mc3<1>(input_data[5], input_data[2], input_data[1], input_data[0],
						half_layer1[5], half_layer1[2], half_layer1[1], half_layer1[0],
						inv[2], inv[1], inv[0],
						modulus[5], modulus[2], modulus[1], modulus[0]);

	ntt_mc3_new(input_data[2], input_data[1], input_data[0],
				rp[2], rp[1], rp[0],
				srp[2], srp[1], srp[0],
				modulus[2][0], modulus[1][0], modulus[0][0]);
...
#pragma HLS DEPENDENCE dependent=false type=inter variable=data_out0
				...
				data_out0[idx + k * MODCOUNT2 *N + m * N]((0 + 1) * BITWIDTH - 1, 0 * BITWIDTH) = input_data[m][k][i][j];
				//out_idx++;
			}
