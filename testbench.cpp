...
int main() {
  ...
  for (int i = N; i < DATA_IN0; i++) {
    data_in0[i]((0 + 1) * BITWIDTH - 1, 0 * BITWIDTH) =
        cipher_data[2 * N + i - N];
    data_in0[i]((1 + 1) * BITWIDTH - 1, 1 * BITWIDTH) =
        cipher_data[8 * N + i - N];
    data_in0[i]((2 + 1) * BITWIDTH - 1, 2 * BITWIDTH) =
        plain_data[2 * N + i - N];

    data_in1[i]((0 + 1) * BITWIDTH - 1, 0 * BITWIDTH) =
        cipher_data[1 * N + i - N];
    data_in1[i]((1 + 1) * BITWIDTH - 1, 1 * BITWIDTH) =
        cipher_data[7 * N + i - N];
    data_in1[i]((2 + 1) * BITWIDTH - 1, 2 * BITWIDTH) =
        plain_data[1 * N + i - N];

    data_in2[i]((0 + 1) * BITWIDTH - 1, 0 * BITWIDTH) =
        cipher_data[0 * N + i - N];
    data_in2[i]((1 + 1) * BITWIDTH - 1, 1 * BITWIDTH) =
        cipher_data[6 * N + i - N];
    data_in2[i]((2 + 1) * BITWIDTH - 1, 2 * BITWIDTH) =
        plain_data[0 * N + i - N];
  }
