...
void intt_4core_new(UDTYPE operand[bramnum][bramsize],
                    ...
                    UDTYPE sidt[RPBRAMNUM][RPBRAMSIZE], UDTYPE modulus) {
...
#pragma HLS ARRAY_PARTITION variable = sw complete dim = 1
      ...
      // 			{
      // #pragma HLS UNROLL
      // 				uint32_t x = (arrayWindex[m] +
      // temp_adder);
      // 				// w[m]=rp[x];
      // 				//
      // sw[m]=srp[x]; 				w[m] = idt[0]; 				sw[m] = sidt[0];
      // 			}
...
#pragma HLS UNROLL
          ...
          inverse_ntt_core(w[m], sw[m], modulus, two_times_modulus, operanda[m],
...
#pragma HLS UNROLL
          ...
          //endl;
          //
          //					cout << "in: operanda[m]" << operanda[m] <<
          //endl; 					cout << "in: operandb[m]" << operandb[m] << endl;
          inverse_ntt_core(w[m], sw[m], modulus, two_times_modulus, operanda[m],
                           operandb[m], operanda_[m], operandb_[m]);

          //					cout << "out: operanda[m]" << operanda_[m] <<
          //endl; 					cout << "out: operandb[m]" << operandb_[m] << endl;
        }
...
#pragma HLS UNROLL
        ...
        //				cout << "MEa" << MEa << endl;
        //				cout << "MEb = " << MEb << endl;
        //				cout << "[m][MEa] = " << operand[m][MEa] <<
        //endl; 				cout << "[m][MEb] = " << operand[m][MEb] << endl;
...
#pragma HLS UNROLL
        /*
                                        if (i == stage_tempa){
                                                ...
                                                cout << "in: operanda[m]" <<
           operanda[m] << endl; cout << "in: operandb[m]" << operandb[m] <<
           endl;
                                        }*/

        inverse_ntt_core(w0, sw0, modulus, two_times_modulus, operanda[m],
                         operandb[m], operanda_out[m], operandb_out[m]);
        ...
        inverse_ntt_core(w00, sw00, modulus, two_times_modulus, operanda_[m],
        ...
        // inverse_ntt_negacyclic_harvey_lazy_core(w0, sw0, modulus,
        // two_times_modulus, operanda[m], operandb[m]);
      }
      j++;
...
#pragma HLS UNROLL
      ...
      for (int m = 0; m < corenum; m++) {
#pragma HLS UNROLL
        operand[m][MEa] = static_cast<UDTYPE>(operanda_out[m]) -
                          (modulus & static_cast<UDTYPE>(-static_cast<DTYPE>(
                                         operanda_out[m] >= modulus)));
        operand[m][MEb] = static_cast<UDTYPE>(operandb_out[m]) -
                          (modulus & static_cast<UDTYPE>(-static_cast<DTYPE>(
                                         operandb_out[m] >= modulus)));
        operand[m + corenum][MEa] =
            static_cast<UDTYPE>(operanda_2out[m]) -
            (modulus & static_cast<UDTYPE>(
                           -static_cast<DTYPE>(operanda_2out[m] >= modulus)));
        operand[m + corenum][MEb] =
            static_cast<UDTYPE>(operandb_2out[m]) -
            (modulus & static_cast<UDTYPE>(
                           -static_cast<DTYPE>(operandb_2out[m] >= modulus)));
